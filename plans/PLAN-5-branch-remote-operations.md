...
| Fetch prunes | **`git fetch --prune`**, not a separate `git remote prune`: that command is itself a network round trip, so doing it first would cost two connections and two auth handshakes per refresh. Without pruning, a branch someone else deleted keeps its tracking ref forever and keeps appearing in the menu, offering a checkout of something that no longer exists. |
| …and overrides `fetch.prune=false`, but only where that is safe | Pruning is **not** confined to `refs/remotes/*`: it deletes stale refs at every destination the remote's refspecs cover, and `remote.<n>.fetch` is user-editable. With `+refs/tags/*:refs/tags/*` configured, a prune deletes the user's local-only tags; with `+refs/notes/*:refs/notes/*` it deletes `refs/notes/commits` and the note content with it. Both verified against real git, and in both cases `fetch.prune=false` is the only guard. So the override is **gated on the refspec**: `--prune` is forced only when every destination is under `refs/remotes/`, which is every ordinary clone. Anything more exotic keeps the user's own config and its protection. Within that gate the override is deliberate — someone who set `fetch.prune=false` set it for `git fetch` on the command line, not for a button whose meaning is "make my view of the remote accurate". |
| A pruned upstream | Reported as **`upstreamGone`**, shown as "upstream gone", with Pull disabled and Push offering to republish. `%(upstream:short)` still names the upstream after its ref is pruned (it comes from config), so ahead/behind come back 0/0 — which would render as a reassuring `↑0 ↓0` for a branch whose remote copy has been deleted. `%(upstream:track)` does say `[gone]`, but that is prose; the full `%(upstream)` ref is verified with `rev-parse --verify` instead. |
| A local-branch upstream | `git branch --track topic main` sets `branch.topic.remote = "."`, a valid upstream that is a local branch. Reported as **`upstreamOnRemote: false`** and treated by the sync controls as unpublished, since it is not a remote relationship: without it a missing *local* branch would be described as gone "from the remote" and Publish would silently repoint the branch at `origin`. |
