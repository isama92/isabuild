...
// it. A diff or merge window has an editor instead, and Monaco and CodeMirror
// both consume keys they have a use for and mark them handled — Escape closes
// Monaco's find bar, CodeMirror's autocomplete and so on. Listening in the
// bubble phase, and skipping anything already handled, is what lets Escape mean
// "dismiss the widget" first and "close the window" only when there was no
// widget.
//
// Ctrl/Cmd+W is not routed through here: see the note in `lib/keybindings`.

export type WindowAction = "close-window" | "next-conflict" | "previous-conflict";
