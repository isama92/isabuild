...
- **Multiple windows**: four documents, one Vite entry each — `index.html` (the `main` workspace), `diff.html`, `merge.html`, `settings.html`. A secondary window must never mount the workspace Layout, or opening a file would spawn a second Claude Code PTY; separate entries are what enforce that. A diff or merge window is **one per file**, with a label that is a pure function of the file (`lib/fileWindow.ts`: `<prefix>-<slug>-<hash>`), so opening the same file twice focuses the existing window instead of making another. Window-scoped behaviour keys off the label in Rust and off the entry module in the frontend, and **a new label pattern needs its own file in `src-tauri/capabilities/`** — `core:window:default` grants getters only, so `close`/`destroy`/`set-focus` must be listed explicitly.
...
- **Events over polling**: repo changes come from the `notify` watcher (debounced 300 ms) emitting `repo://changed`; the frontend re-requests state. No timers polling git. Two filters sit in front of that event and both are load-bearing: `watchfilter` decides in five passes, cheapest first, asking `git check-ignore` only as a last resort, so a build directory costs one question ever; and a path whose remembered mtime, length, ctime and mode are unchanged is discarded, because `notify`'s inotify mask includes `OPEN` and every file `git status` reads is otherwise an event that asks for the next read.
- **Editors**: CodeMirror 6 is the editor stack. Everything shared by the diff and merge windows lives in `src/editor/` — the theme and pane extensions (`codemirror.ts`), the window chrome and its hook (`EditorWindow`, `useEditorWindow`), the toolbar (`EditorToolbar`), the change strip, and the view-option registry. A new editor window composes those rather than hand-rolling chrome, and a new **toolbar view setting is one entry in `viewOptions.ts` plus one handler in the pane that implements it** — modelled on `keybindings.ts`, overrides-only in `config.json`. The diff panes are a `@codemirror/merge` `MergeView`, which has three sharp edges, each commented where it bites: the **container scrolls, not the editors** (so `.cm-mergeView { height: 100% }` in `diff.css` is what keeps the viewport virtualised), `EditorView.editable.of(false)` **costs a pane Ctrl+F** because a keystroke never reaches something that cannot take focus, and its `diffConfig` needs a `timeout` rather than the documented `scanLimit`, whose bound is counted in characters and silently returns one chunk for the whole file. The **merge window aligns its three panes the same way but with its own machinery**, because that aligner is private to `MergeView` and a `MergeView` is strictly two documents: one container scrolls all three editors (each at `height: auto`, headers sticky), and the padding is block spacer widgets from `lib/mergeAlign`, which is pure line arithmetic and knows nothing about pixels beyond the one line height the component multiplies by. There is no scroll sync anywhere, and reintroducing one would mean the alignment had failed.
...
- Work in small steps; the todo list in README.md is the backlog **and the only planning document**. A finished part is **deleted** from it and the remaining parts renumbered from 1 — the numbers are queue positions, not ids, so what shipped lives in `CHANGELOG.md` and the git history instead (code comments keep whatever number a part had at the time). A part's rationale, scope and acceptance criteria live in its own todo entry — write and refine them there. Do **not** create plan files or a `plans/` directory; detailed planning happens in conversation, and whatever is durable goes into the entry. When a requirement is ambiguous or a decision shapes UX or architecture, ask before building.
...
- **jsdom cannot measure.** Every box is zero, so anything positional — an editor's line geometry, a context menu's placement, a change strip's marks — has to be either a pure function over an *injected* geometry (`lib/overviewStripes`, `lib/mergeAlign`, `lib/contextMenu`) or a stub of the few numbers involved. Keep the arithmetic outside the component so it can be tested at all, say in the test's header what it therefore cannot assert, and leave the rest to the manual pass. What must not happen is a test that passes because nothing was rendered.
- Before committing a completed step, run a read-only review subagent over the uncommitted changes (`git status` / `git diff` / `git diff --staged`). Brief it explicitly (it has none of this conversation's context): what the feature does, its acceptance criteria, and to follow this CLAUDE.md. It reports only, never edits. Then triage: fix real issues, skip false positives, note your calls. Re-run the suites if a fix touched code, then commit. (`/ship` runs this flow — `.claude/commands/ship.md`.)
- Commit per completed step with a descriptive message; the pre-commit hook must pass.
- PR titles must be Conventional Commits (`feat:`, `fix:`, `feat!:`, `ci:`, `chore:`, …), enforced by `conventions.yml`. PRs are squash-merged, so the title becomes the commit on `main` that release-please reads to pick the next version. A todo part is titled `feat: part <its current number> — <name>`. Because finished parts are removed and the rest renumbered, those numbers repeat over time; the name is what identifies the work.
...
- A todo part is only removed from the list when its acceptance criteria are verified on macOS, Linux and Windows (ask the user to confirm platforms you cannot test yourself). Until then it stays, with what is outstanding said in the entry.
