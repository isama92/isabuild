...
- **Editors**: CodeMirror 6 is the only editor stack (Part 10 retired Monaco). Everything shared by the diff and merge windows lives in `src/editor/` — the theme and pane extensions (`codemirror.ts`), the window chrome and its hook (`EditorWindow`, `useEditorWindow`), the toolbar (`EditorToolbar`), the change strip, and the view-option registry. A new editor window composes those rather than hand-rolling chrome, and a new **toolbar view setting is one entry in `viewOptions.ts` plus one handler in the pane that implements it** — modelled on `keybindings.ts`, overrides-only in `config.json`. Two traps worth knowing before touching the diff panes are written up in README's Part 10 entry: inside a `MergeView` the *container* scrolls, not the editors, and `EditorView.editable.of(false)` costs you Ctrl+F because an unfocusable pane never receives the key.
...
- PR titles must be Conventional Commits (`feat:`, `fix:`, `feat!:`, `ci:`, `chore:`, …), enforced by `conventions.yml`. PRs are squash-merged, so the title becomes the commit on `main` that release-please reads to pick the next version. A roadmap part is titled `feat: part <its current number> — <name>`. Because finished parts are removed and the rest renumbered, those numbers repeat over time; the name is what identifies the work.
...
- A roadmap part is only removed from the list when its acceptance criteria are verified on macOS, Linux and Windows (ask the user to confirm platforms you cannot test yourself). Until then it stays, with what is outstanding said in the entry.
